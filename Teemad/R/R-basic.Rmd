...
my_sum <- function(a, b=NULL) {
  r = a + b # võib kasutada ka võrdusmärki, kuid konventsionaalselt ikkagi <-
...
# mixed_v = c(1, "õun", TRUE) #stringi ei saa teisendads milleksiki muuks, R konventeerib kõik sellele tüübile, millele on võimalik konventeerida, v.a. siis, kui sees on string
# print(mixed_v)
```
...
```{r}
m <- matrix(1:9, nrow = 3, ncol = 3) # kõik väärtused peavad olema sama tüüpi
print(m)
```

```{r}
# R koodi aken tekitatakse, kui ma visual aknas vali märgi / ja valin R Chunk
...
%<% pivot_longer(cols = c('Katse1', 'Katse2', 'Katse3'), names_to = 'Katse', values_tp = 'tulemus')
print(m)
```
...
# tidyversei käsud on alakriipsuga (read_csv). R enda omad punktiga (read.csv)
```

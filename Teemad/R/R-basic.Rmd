...
```{r}
# Muutujad 
#funktsioonidele antakse väärtused <- ga
x <- 10
y <- 20
x + y
...
mean(c(1, 2, 3, 4, 5))
max(c(1, 2, 3, 4, 5))
# NB! pea meeles 'c()' - ühendab väärtused vektoriks

# funktsioon nimeliste argumentidega
# nimelistele argumentidele antakse väärtused = märgiga, kui väärtused antakse sulgude sees (väljaspool sulge kasutatakse <-)
my_sum <- function(a, b=NULL) {
  
  r <- a + b # väärtuse omistamine
  r = a + b # võib kasutada ka võrdusmärki, kuid R-s kasutatakse ikkagi <-
  
  return(r)
}
my_sum(5, 7) # või my_sum(a=5, b=7)
```
...
total <- function(n=10) { # n - osalejate arv
  total_party_cost <- (food_cost_per_person + drink_cost_per_person) * n + venue_cost + entertainment_cost
  return(total_party_cost)
}   
#{} sisse kirjutatakse pika funktsiooni erinevad väärtused, aga võid kogu valemi kirjutada ka ühele reale
total() # see käsk näitab alles tulemust! See PEAB olema väljaspool funktsiooni
```


```{r}
# Turvalisem (soovitatav) versioon — kõik väärtused argumendina, ei sõltu globaalsest keskkonnast:
total_party_cost <- function(n = 10,
                             venue_cost = 100,
                             food_cost_per_person = 15,
                             drink_cost_per_person = 5,
                             entertainment_cost = 50) {
  if(!is.numeric(n) || n < 0) stop("n peab olema mitte-negatiivne number")
  total <- (food_cost_per_person + drink_cost_per_person) * n + venue_cost + entertainment_cost
  return(total)
}

 total_party_cost() # see käsk näitab alles tulemust! See PEAB olema väljaspool funktsiooni
```
...
1.  **vector**: Ühemõõtmeline elementide massiiv, kus kõik elemendid on sama
    tüüpi. Vektoreid kasutatakse sageli aritmeetilisteks teheteks ja väärtuste järjestamiseks.

```{r}
v <- c(1, 2, 3, 4, 5)
print(v)

# mixed_v = c(1, "õun", TRUE) #stringi ei saa teisendada milleksiki muuks, R konventeerib kõik sellele tüübile, millele on võimalik konventeerida, v.a. siis, kui sees on string
# print(mixed_v)
```
...
```{r}
m <- matrix(1:15, nrow = 3, ncol = 5) # kõik väärtused peavad olema sama tüüpi
print(m)
```

```{r}
# R koodi aken tekitatakse, kui ma visual aknas valin märgi / ja valin R Chunk
m <- t(matrix(1:9, nrow = 3, ncol = 3)) # t - transpose, muudab maatrikisi ümber
print(m)
...
rownames(m) <- c('Pille', 'Malle', 'Mikk')
colnames(m) <- c('Katse1', 'Katse2', 'Katse3')

m <- as.data.frame(m)

m['Nimi'] <- rownames(m)

#tulbad, mida tahan teha pikemaks

# R-s käis torujuhe vanemal kujul %>% magrittr paketist
# %>% pivot_longer(cols = c('Katse1', 'Katse2', 'Katse3'), names_to = 'Katse', values_to = 'tulemus'). 
#print(m)



# Uus moodus on järgmine:
# Selle funktsiooni jaoks tuleb laadida järgmine pakett:
install.packages("tidyr")
library(tidyr)
m_long <- m |>
  pivot_longer(cols = c("Katse1", "Katse2", "Katse3"),
               names_to = "Katse",
               values_to = "tulemus")
print(m_long)
...
Eelnevale lisaks: Tulemuste visualiseerimine.

# Kui ggplot2 pole veel installitud, siis esmalt:
# install.packages("ggplot2")

library(ggplot2)

# Kasutame eelnevalt tehtud m_long tabelit
# m_long peab olema selline: Nimi, Katse, tulemus

# Lihtne punktdiagramm
ggplot(m_long, aes(x = Katse, y = tulemus, color = Nimi)) +
  geom_point(size = 3) +           # punktid
  geom_line(aes(group = Nimi)) +   # jooned iga nime järgi
  theme_minimal() +                # puhas kujundus
  labs(title = "Katsete tulemused", y = "Tulemus", x = "Katse")
  
  Mis see teeb:
aes(x = Katse, y = tulemus, color = Nimi) – seab, et x-telg on katse, y-telg on tulemus, värv näitab osalejat.

geom_point() lisab punktid.

geom_line(aes(group = Nimi)) ühendab sama inimese tulemused joontega.

theme_minimal() teeb graafiku minimalistlikuks.

Tulemuseks on ilus graafik, kus näed kõigi kolme inimese tulemusi kõigi katsete lõikes.
...
# siin antakse funktsioonile argumendid, seega kasuta = märki
print(df)
```
...
# ära aja segi Phytoni listiga - R-s saab sisestada kõiksugu erinevaid andmetüüpe. 
#Kui tahad, et erinevaid muutujad tagastakase ühes elemendis
print(l)
```
...
```{r}
df <- data.frame(
  Aine = c('Bioloogia', 'Bioloogia', 'Bioloogia', 'Keemia', 'Keemia', 'Keemia', 'Füüsika' , 'Füüsika',  'Füüsika'),
...
Maatriksina:
```{r}
# Kõik andmed ühte maatriksisse (tüübid on kõik stringid)
m <- matrix(c(
  'Bioloogia', 20, 85,
  'Bioloogia', 21, 78,
  'Bioloogia', 19, 90,
...
# Lisame veerunimed
colnames(m) <- c("Aine", "Vanused", "Tulemused")
...
```{r}
df <- data.frame(
  Nimi = c("Anna", "Mike", "Luis", "Sam", "Ella", "Ravi", "Mia", "Jack", "Olivia"),
  Klass = c("Bioloogia", "Bioloogia", "Bioloogia", "Keemia", "Keemia",
    "Keemia", "Füüsika", "Füüsika", "Füüsika"),
  Vanused = c(20, 21, 19, 22, 20, 23, 24, 22, 25),
  Tulemused = c(85, 78, 90, 88, 76, 95, 87, 80, 92)
)
...
# Variant 1: kasutame subset() ja Filtreerime tulemuse üle 85
df_over85 <- subset(df, Tulemused > 85, select = c(Nimi, Klass, Tulemused))

print(df_over85)

# Variant 2: kasutame indeksit ja filtreerime tulemuse üle 85
df_over_85 <- df[df$Tulemused > 85, c("Nimi", "Klass", "Tulemused")]
print(df_over_85)
...
# Laadi tidyverse
library(tidyverse)
#kontrollimiseks sessionInfo() või packageVersion("tidyverse")
...
kui sa jooksutad:
data_csv <- read_csv("../../data/Islander_data.csv")
siis R loeb CSV-faili andmed mällu ja salvestab need objekti data_csv.

data_csv on nüüd tibble (tibble on tidyverse’i modernsem data.frame).

Kui kirjutad lihtsalt data_csv, kuvab R tabeli esimesed read automaatselt.

Märkus:
read_csv() on tidyverse / readr funktsioon, mille käsud on alakriipsuga (read_csv, write_csv).

R enda variant on read.csv(), mille käsud on punktidega (read.csv, write.csv).

Näiteks: # Base R
data_csv_base <- read.csv("../../data/Islander_data.csv")
...
Tidyverse toru-operaator, mida tähistab `%>%` \<ctrl+shift+M\>, võimaldavad mitut funktsiooni järjestada selgelt ja loetavalt. 

Toru-operaator võtab ühe funktsiooni väljundi ja annab selle järgmise funktsiooni sisendiks, muutes andmetöötluse järjestuse lihtsasti jälgitavaks.
...
# pipe-operaator %<% - on võimalik erinevaid operatsioone liigendada üksteise järgi, tuleneb Linuxist
```
...
`filter()` võimaldab `data.frame`-i ridade alamhulka valida, jättes alles read, mis vastavad tingimusele.
...
Järgmises näites kasutame toru, et teha järjestikuseid toiminguid: 

1. Rühmitame andmed kategooria järgi (`group_by`) 
2. Arvutame iga rühma kohta kokkuvõtlikud näitajad (`summarize`) 
3. Sorteerime tulemused kindla näitaja järgi (`arrange`)
...
Andmete ümberkujundamine on tavaline analüüsitöö. 
`tidyr` paketis on kaks võimsat funktsiooni: `pivot_longer()` ja `pivot_wider()`. 
Need võimaldavad muuta andmete paigutust "pika" ja "laia" formaadi vahel.
...
`relig_income` andmestikus on tulunivood veergude nimedena. Saame kõik veerud peale `religion` muuta pikaks formaadiks. 
`!religion` valib kõik veerud peale `religion`. 
`names_to = "income"` loob uue veeru nimega "income" veerunimede jaoks, `values_to = "count"` salvestab väärtused.
...
head(data_csv)
```
Kui tahad tabelit ilusamana näha:
library(tidyverse)

print(data_csv)       # kuvab tibble kujul
View(data_csv)        # avab RStudios eraldi tabeliaknas
glimpse(data_csv)     # kiire ülevaade struktuurist
...
tulemus <- data_csv %>%
  group_by(Drug) %>%                                 # rühmita ravimi järgi
  summarise(
    keskmine_Diff = mean(Diff, na.rm = TRUE),        # 1. keskmine Diff
    osalejate_arv = n()                              # 2. mitu inimest
  ) %>%
  arrange(desc(keskmine_Diff))                       # 3. sorteeri kahanevalt
...
tulemus <- data_csv %>%
  group_by(Drug, Dosage) %>%               # rühmita ravimi ja doosi järgi
  summarise(
    keskmine_Diff = mean(Diff, na.rm = TRUE),        # 1. keskmine Diff
    .groups = "drop"                        # eemaldame rühmitamise pärast
  ) %>%
 arrange(Drug, desc(keskmine_Diff))   # sorteerime esmalt Drug, siis Diff kahanevalt

print(tulemus)
```
Mis siin toimub:
group_by(Drug, Dosage) → teeme grupid iga ravimi ja doosi kombinatsiooni järgi.
summarise(keskmine_Diff = mean(Diff, na.rm = TRUE)) → arvutame iga grupi keskmise Diffi.
.groups = "drop" → aitab vältida hoiatusi, eemaldab rühmitamise pärast kokkuvõtet.
arrange(Drug, desc(keskmine_Diff)) → tulemused järjestatakse kõigepealt ravimi järgi ja ravimi sees Diff kahanevalt.
...
#rühmitamine ja keskmiste arvutamine
summary_df <- data %>% # siin data, sest enne tegin data_csv-st data
...
```{r}
#rühmitamine ja keskmiste arvutamine
summary_df <- data_csv %>% # siin data_csv, sest ei ole teinud data_csv-st data
